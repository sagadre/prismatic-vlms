...
def pretrain(cfg: PretrainConfig) -> None:
    ...
    cfg.run_id = f"{cfg.model.model_id}+stage-{cfg.stage}+x{cfg.seed}" if cfg.run_id is None else cfg.run_id
    ...
    overwatch.info(f"Instantiating VLM `{cfg.model.model_id}` for Training Stage `{cfg.stage}`")
    vlm = (
    ...
    overwatch.info(f"Invoking `VLM.freeze_backbones()` for `{cfg.model.model_id}` => Training Stage: `{cfg.stage}`")
